gen_data_signal_detection_noufaily_2012 <- function(seed = 4){
  ...
  d <- cstidy::csfmt_rts_data_v1(data.table(
    ...
    location_code = "norge",
    date = do.call(c, cstime::dates_by_isoyearweek[isoyear %in% c(2015:2023)]$days),
    age = "total",
    sex = "total",
    border = 2020
  ))
...
# x = gen_data_signal_detection_noufaily_2012()
# numerator = "cases_n"
...
signal_detection_noufaily_2012_internal <- function(
  ...
  denominator = NULL,
  prX = 100,
  training_years_seasons = 5,
  training_type = "years",
  remove_last_dates = 0,
  forecast_dates = 42,
  remove_last_isoyearweeks = ceiling(remove_last_dates / 7),
  forecast_isoyearweeks = ceiling(forecast_dates / 7),
  ...
  if(gran_time=="isoyearweek"){
    ...
    # daily
    # if(training_dates < 14){
    #   stop("training_dates must be >= 14 dates when granularity_time is date")
    # }
  ...
  varname_forecast_predinterval_q00x5_numerator <- paste0(prefix, "_forecasted_predinterval_q00x5", suffix)
  varname_forecast_predinterval_q99x5_numerator <- paste0(prefix, "_forecasted_predinterval_q99x5", suffix)
  varname_forecast_numerator <- paste0(prefix, "_forecasted", suffix)
  if(!is.null(denominator)){
    ...
    varname_forecast_predinterval_q00x5_prX <- paste0(prefix, "_forecasted_predinterval_q00x5_pr", formatC(prX, format="f", digits = 0))
    varname_forecast_predinterval_q99x5_prX <- paste0(prefix, "_forecasted_predinterval_q99x5_pr", formatC(prX, format="f", digits = 0))
  ...
  with_pred_split <- split(with_pred, with_pred$isoyear)

  seqs_outcome <- seq_along(with_pred_split)
  seqs_training <- vector("list", length = length(with_pred_split))
  for(i in 1:length(seqs_training)){
    if(i<=6){
      seqs_training <- 1:5
    } else {
      seqs_training <- (i-5):i
    ...
    training <- rbindlist(with_pred_split[seqs_training], fill=T)
    formula <- glue::glue("{varname_forecast_numerator} ~ trend_variable")
    fit <- glm2::glm2(as.formula(formula), data = training, family = "poisson")
    ...
    with_pred_split[[i]][, (varname_forecast_numerator) := p$lower]
    with_pred_split[[i]][, (varname_forecast_predinterval_q00x5_numerator) := p$point]
    with_pred_split[[i]][, (varname_forecast_predinterval_q99x5_numerator) := p$upper]
  }

  final <- rbindlist(with_pred_split)
