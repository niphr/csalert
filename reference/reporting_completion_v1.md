...
\`complete_by_md\`, and \`pct_delay0\`..\`pct_delay\<max_delay-1\>\`.
Each \`pct_delayD\` is the pooled % of cases reported by the end of week
reference + D – the delay ECDF, no interpolation. \`pct_delay0\` is the
reference week itself, NOT the week after. Every one of these is
computed AFTER delays \`\>= max_delay\` are discarded. They describe the
cases that arrive within the horizon, not all eventual cases.
