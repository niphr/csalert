...
test_that("family = 'identity' reproduces the pre-change trend exactly", {
  # Captured from csalert 2026.8.23 (commit ed0749b), before `family` existed.
  # Any drift in the default path lands here, including a shift in the rt()
  # stream that propagate_slope_error draws from.
  a <- short_term_trend(ens_2draw(), measure = "cases", trend_isoyearweeks = 3)
  expect_identical(
    a$draws[["cases_trend_beta1"]],
    structure(
  ...
  expect_identical(
    a$data[["cases_trend_increasing_pr"]],
    c(NA, NA, 1, 1, 0.5, 1, 1, 1)
  )
...
test_that("family = 'quasipoisson' matches a per-window glm, zeros included", {
  ...
  for (y in list(
    ...
    expect_equal(
      out$draws[["cases_trend_beta1"]][, 1],
      ref$beta1,
      tolerance = 1e-6
    )
    expect_equal(
      out$draws[["cases_trend_gr"]][, 1],
      100 * (exp(ref$beta1) - 1),
      tolerance = 1e-6
    )
    expect_equal(
...
test_that("propagate_slope_error un-degenerates P(increasing) at width 3", {
  # Every 3-week window here zig-zags, so the slope is small next to its own
  # standard error: max |beta1 / se| is 0.43. A GLM Wald interval is
  # asymptotic, so the perturbation is normal and width 3 stays usable. The
  # identity path would be a t_1 here, which is why it refuses width 2 and
  # warns about width 3.
  y <- c(10, 13, 11, 14, 12, 15, 13, 16, 14, 17, 12, 16)

  off <- short_term_trend(
    ens_1draw(y),
    measure = "cases",
    trend_isoyearweeks = 3,
    family = "quasipoisson"
  )
  inc_off <- off$data[["cases_trend_increasing_pr"]][3:length(y)]
  expect_true(all(inc_off %in% c(0, 1)))
...
test_that("a positional call still binds trend_isoyearweeks and propagate_slope_error", {
  # `family` and `denominator` were appended after `n_sim`. Argument 3 stays
  # `trend_isoyearweeks` and argument 4 stays `propagate_slope_error`. Two
  # signals pin that. Width 3 leaves exactly two leading NA rows, and
  # `propagate_slope_error = TRUE` widens the draw axis to `n_sim`.
  y <- c(4, 6, 5, 9, 11, 10, 14, 19, 17, 26, 31, 29)

  set.seed(3)
  pos <- short_term_trend(ens_1draw(y), "cases", 3, TRUE)
  set.seed(3)
  ...
  expect_identical(ncol(b), 1000L) # the default n_sim, so the flag was TRUE
})
...
test_that("propagate_slope_error refuses width 2 wherever the dispersion is estimated", {
