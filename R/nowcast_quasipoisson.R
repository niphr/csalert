...
# weeks, a quasipoisson regression with an identity link:
#
#     total  ~  a + b0 * n[delay 0] + b1 * n[delay 1] + ... + bh * n[delay h]
#
# i.e. "each case reported at delay d implies, on average, b_d cases in total",
# plus a constant `a`. The intercept IS fitted: the formula is built as
# `y ~ d1 + d2 + ...`, which carries R's default intercept, and `start` supplies a
# 0 for it. There is NO per-week magnitude parameter (the thing that made the
# chain-ladder noisy for exactly the recent weeks); the b's are shared and learned
# from many complete weeks, then applied to the incomplete weeks at that horizon.
# Draws come from the fit's prediction uncertainty (parameter) + a
# dispersion-matched negbin (observation). Whether the resulting intervals are
# CALIBRATED is an empirical question about a particular series, not a property of
# this construction -- measure it with nowcast_evaluate_v1(). delay_window keeps
...
.glm_complete <- function(mat, refs, as_of, max_delay, n_sim, delay_window) {
  ...
  as_of_i <- match(as_of, weeks)
  ref_i <- match(refs, weeks)
  ...
  train <- which(
    settled &
      (if (is.null(delay_window)) TRUE else age < delay_window + max_delay)
  )
  if (length(train) < 3L) {
    return(draws)
  }
...
#' the contract `f(reporting_triangle, ...) -> csfmt_ensemble_v3`.
